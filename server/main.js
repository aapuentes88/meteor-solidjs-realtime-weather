  ...
  Meteor.methods({
    async initMonitor(city) {
      try {
        ...
        Monitor.setForecastIdentifier(
          Meteor.setInterval(forecast, myWeather.forecastInterval)
        );
      } catch (error) {
        console.error('Error al iniciar el monitor:', error);
        throw new Meteor.Error(
          'init-monitor-failure',
